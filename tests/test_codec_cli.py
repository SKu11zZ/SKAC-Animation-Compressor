...
class CodecCliTests(unittest.TestCase):
    def test_v2_encode_inspect_and_decode_commands(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source = root / "source.bvh"
            encoded = root / "motion-v2.skac"
            ...
            self.assertEqual(json.loads(output.getvalue())["format_version"], 2)
            output = io.StringIO()
            with contextlib.redirect_stdout(output):
                self.assertEqual(main(["inspect", str(encoded)]), 0)
            self.assertEqual(json.loads(output.getvalue())["format_major"], 2)
